...
describe('Flow Inventory Unit Quotation', () => {
  it('passed', () => {
    ...
    cy.intercept('POST', '**/inventory/quotations').as('quotationCreateCall')
    cy.intercept('GET', '**/inventory/quotations/*').as('quotationDetailCall')

    cy.Login(Cypress.env('EMAIL_UNIT_USER'), Cypress.env('PASSWORD')).then(() => {
      cy.get('#sidebar_sales_order').click({ force: true })
      cy.get('#submenu_quotations').click({ force: true })
    })
    cy.reload().then(() => {
      cy.get('body').click()
    })
    ...
    cy.get('#input_quotation_number').type(`QUO-${additionalVal}`)
    cy.get('#input_quotation_date').type('2024-01-25')
    cy.get('#input_valid_until').type('2024-02-25')
    ...
    cy.wait('@quotationCreateCall').then(({ response }) => {
      expect(response.statusCode).to.eq(200)
    })
    //DETAIL
    ...
    cy.wait('@quotationDetailCall').then(({ response }) => {
      expect(response.statusCode).to.eq(200)
    })
    cy.get('#input_customer_type').should('be.disabled')
    cy.get('#input_shipping_point').should('be.disabled')
    cy.get('#input_quotation_number').should('be.disabled')
    cy.get('#input_quotation_date').should('be.disabled')
    cy.get('#input_valid_until').should('be.disabled')
